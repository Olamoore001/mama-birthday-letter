(function () {
  ...
  function wait(ms) { return new Promise(function (res) { setTimeout(res, prefersReducedMotion ? Math.min(ms, 60) : ms); }); }

  /* ============================================================
     SCENE / STATE MANAGEMENT
     ============================================================ */
  var sceneOrder = ['opening', 'envelope', 'letter', 'thankyou', 'prayer', 'secret', 'birthday', 'lastthing', 'final'];
  var scenes = {};
  sceneOrder.forEach(function (name) { scenes[name] = qs('[data-scene="' + name + '"]'); });
  ...
  function transitionToScene(name, onEnter) { showScene(name, onEnter); }
  ...
  function startMusic() {
    ...
    if (p && typeof p.catch === 'function') {
      p.catch(function () {
        // Autoplay blocked or file missing — fail silently, rest of site keeps working.
        musicStarted = false;
      });
    }
    fadeMusic(0.32, 2000);
  ...
  function sparkleBurst(count) {
    for (var i = 0; i < count; i++) {
      (function (i) {
        setTimeout(function () { createSparkle(35 + Math.random() * 30, 24 + Math.random() * 22); }, i * 90);
      })(i);
    }
  }
  ...
  function createConfetti(durationMs) {
    ...
    var elapsed = 0, tick = 140;
    confettiInterval = setInterval(function () {
      createConfettiPiece();
      createConfettiPiece();
      elapsed += tick;
      if (elapsed >= durationMs) { clearInterval(confettiInterval); confettiInterval = null; }
    }, tick);
  }
  ...
  function revealSequentially(container, selector, stepMs, startDelay) {
    ...
    items.forEach(function (el, i) {
      setTimeout(function () { el.classList.add('is-shown'); }, (startDelay || 0) + i * (stepMs || 350));
    });
  ...
  function animateLetter() {
    ...
    setTimeout(function () { qs('#scene-letter .paper-continue').classList.add('is-shown'); }, total + 300);
  }
  ...
  qsa('[data-next]').forEach(function (btn) {
    btn.addEventListener('click', function () { routeTo(btn.getAttribute('data-next')); });
  });
  ...
  function routeTo(next) {
    ...
    else if (next === 'secret') transitionToScene('secret', startSecretTransition);
    else if (next === 'lastthing') transitionToScene('lastthing', function () { stopAmbientPetals(); });
  }
  ...
  function showThankYou() {
    ...
    setTimeout(function () { body.classList.add('is-shown'); }, lineDelay + 200);
    var btn = qs('#scene-thankyou .btn--ghost');
    setTimeout(function () { btn.classList.add('is-shown'); }, lineDelay + 900);
    setTimeout(function () { sparkleBurst(6); }, lineDelay + 200);
  }
  ...
  function showPrayer() {
    ...
    setTimeout(function () { qs('#prayerAmen').classList.add('is-shown'); }, pDelay + 500);
    setTimeout(function () { qs('#prayerContinueBtn').classList.add('is-shown'); }, pDelay + 1300);
  }
  ...
  function startSecretTransition() {
    ...
    (async function () {
      ...
      if (!prefersReducedMotion) {
        for (var i = 0; i < 3; i++) { createSparkle(45 + Math.random() * 10, 42 + Math.random() * 10); await wait(500); }
        createPetal();
        await wait(500);
        createPetal();
      }
  ...
  function showBirthdayReveal() {
    var stage = qs('#stage');
    stage.style.background = 'radial-gradient(ellipse at 50% 25%, #FCEBD1, #F3B98B 65%, #E8875A 130%)';
  ...
  function showFinal() {
    var stage = qs('#stage');
    stage.style.background = 'linear-gradient(180deg, var(--ivory), var(--cream))';
    ...
    setTimeout(function () { qs('#finalSignature').classList.add('is-shown'); }, total + 500);
  }
